...
export default function LawyerCaseDetailsPage() {
  ...
  const isDraft = caseData?.submissionStatus === "draft";
  const hearings = hearingsResult?.data || [];
  const judgment = judgmentResult?.data; // getJudgments returns an array; caseId is unique on Judgment, so at most one entry
  const documents = documentsResult?.data || [];
  const activeJudgment = Array.isArray(judgment) ? judgment[0] : judgment;
              ...
              {judgmentLoading ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm whitespace-pre-wrap leading-relaxed wrap-break-word text-foreground/90">
